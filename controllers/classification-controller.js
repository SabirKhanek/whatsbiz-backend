...
async function get_classification(messages) {
  ...
  messages.forEach((message) => {
    const str = `${message}\n`.replace(/\n/g, " ");
    let cleanStr = iconv.decode(
      iconv.encode(str, "UTF-8", { mode: "replace" }),
      "UTF-8"
    );
    cleanStr += "\n";
  ...
  await fs.promises.writeFile(payloadPath, data, {
    flag: "w",
    encoding: "utf8",
  });
  ...
  spawned = spawn(
    "python3",
    ["intent-classifier.py", "predict", "." + payloadFileName],
    { cwd: classifierPath, shell: true }
  );
